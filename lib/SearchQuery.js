...
// Make the sql object available for convenience.
var Sql = opDataSet.sql;
...
function augmentQueryWithElement(self, element, index) {
  switch (index) {
    case 'gender':
      ...
      break;
    case 'age':
      // Transform any incoming array data to integers for age.
      var searchArgs = _.map(element, function(val) { return S(val).toInt() });
      // Add 0 to array to include ANY age opportunities.
      searchArgs = _.union(searchArgs, [0]);
      // Search array with array.
      self.query.where('"opportunities"."age" && ' + Sql.array.apply(this, searchArgs).toString());
      break;
    case 'businessType':
      searchArgs = new Array(element);
      searchArgs = _.union(searchArgs, ['any']);
      self.query.where('"opportunities"."eligibleEntityTypes" && '
        + Sql.array.apply(this, searchArgs).toString()
        + '::varchar[]');
      break;
    case 'industry':
      searchArgs = new Array(element);
      searchArgs = _.union(searchArgs, ['any']);
      self.query.where('"opportunities"."eligibleIndustries" && '
        + Sql.array.apply(this, searchArgs).toString()
        + '::varchar[]');
      break;
