...
function SearchQuery(queryBody) {
  ...
  this.query = opDataSet.select(
    opDataSet.id,
    opDataSet.title,
    opDataSet.gender,
    opDataSet.avgApplicationTime.as('estimatedTime'),
    opDataSet.applicationDeadline.as('deadline'),
    opDataSet.benefitDescription.as('description'),
    opDataSet.benefitType,
    opDataSet.purpose.as('purpose'),
    opDataSet.paperworkRequired.as('paperwork'),
    opDataSet.applicationCost.as('cost'),
    opDataSet.additionalGeneralInformation.as('info'),
    opDataSet.title.as('name'), // placeholder for machine name for now
    opDataSet.agencyId,
    // Agency
    agencyDataSet.name.as('agencyName'),
    agencyDataSet.web.as('agencyWeb'),
    agencyDataSet.phone.as('agencyPhone'),
    // Requirement
    reqDataSet.name.as('reqName'),
    reqDataSet.name.as('link'),
    reqDataSet.name.as('cost')
  ).from(
    opDataSet.join(agencyDataSet)
      .on(opDataSet.agencyId.equals(agencyDataSet.id))
    .leftJoin(opportunitiesrequirements)
      .on(opDataSet.id.equals(opportunitiesrequirements.opportunityId))
    .leftJoin(reqDataSet)
      .on(opportunitiesrequirements.requirementId.equals(reqDataSet.id))
  );
