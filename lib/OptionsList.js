...
var choicesOverrideList = {
  fundMeWizard: {
    gender: {
      any: 'Both', // Change label.
...
/**
 * Process form choices - remove various choices or labels for certain forms
 */
function processFormChoices(self) {
  if (_.isUndefined(choicesOverrideList[self.formName]))
    return;

  var overrides = choicesOverrideList[self.formName];
  // Iterate over each field key that overrides provides.
  _.each(overrides, function(optionOverrides, fieldName) {
    console.log(optionOverrides);
    var fieldOptions = self.options[fieldName];
    // Transform array to object for consistency if needed.
    if (_.isArray(fieldOptions))
      fieldOptions = _.zipObject(_.keys(fieldData), fieldData);

    console.log(fieldOptions);
    // Iterate over every single override for a field.
    _.each(optionOverrides, function(overrideValue, overrideKey) {
      if (!_.isUndefined(fieldOptions[overrideKey])) {
        // If explicitly set to NULL, delete the key.
        if (overrideValue === null)
          delete fieldOptions[overrideKey];
        // If value is provided override with value.
        else
          fieldOptions[overrideKey] = overrideValue;
      }
    ...
    // Set the new fieldOptions on the field.
    self.options[fieldName] = fieldOptions;
  });
};
...
OptionsList.prototype.getFormChoices = function(field) {
  ...
  if (_.isArray(fieldData)) {
    formChoices = _.zipObject(_.keys(fieldData), fieldData);
  }
  else {
